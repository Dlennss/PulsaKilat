...
func (r *ProviderReportingRepository) loadProviderAnalyticsPeriods(ctx context.Context, a ProviderAnalyticsArgs, grain string, successExpr string) ([]ProviderAnalyticsPeriodRecord, error) {
	periodExpr := "date_trunc('" + grain + "', t.dibuat_pada)::date"
	depositPeriodExpr := "date_trunc('" + grain + "', mdp.dibuat_pada)::date"
	query := `
WITH period_trx AS (
  SELECT
    t.provider,
    ` + periodExpr + ` AS period_start,
    count(*) FILTER (WHERE ` + successExpr + `) AS success_count,
    coalesce(sum(CASE WHEN ` + successExpr + ` THEN coalesce(t.harga,0) ELSE 0 END),0) AS success_nominal
  FROM public.transaksi_provider t
  WHERE ($1 = false OR t.dibuat_pada >= $2)
    AND ($3 = false OR t.dibuat_pada <  $4)
  GROUP BY t.provider, ` + periodExpr + `
),
period_dep AS (
  SELECT
    mdp.provider,
    ` + depositPeriodExpr + ` AS period_start,
    coalesce(sum(mdp.jumlah),0) AS deposit_amount
  FROM public.mutasi_dompet_provider mdp
  WHERE ($1 = false OR mdp.dibuat_pada >= $2)
    AND ($3 = false OR mdp.dibuat_pada <  $4)
    AND mdp.arah = 'credit'
    AND (
      mdp.alasan = 'PROVIDER_DEPOSIT' OR
      mdp.alasan = 'BANK_TRANSFER_IN' OR
  ...
  GROUP BY mdp.provider, ` + depositPeriodExpr + `
),
periods AS (
  SELECT provider, period_start FROM period_trx
  UNION
  SELECT provider, period_start FROM period_dep
)
SELECT
  p.provider,
  p.period_start,
  coalesce(t.success_count,0) AS success_count,
  coalesce(t.success_nominal,0) AS success_nominal,
  coalesce(d.deposit_amount,0) AS deposit_amount
FROM periods p
LEFT JOIN period_trx t ON t.provider = p.provider AND t.period_start = p.period_start
LEFT JOIN period_dep d ON d.provider = p.provider AND d.period_start = p.period_start
ORDER BY p.period_start DESC, p.provider ASC
`
	rows, err := r.db.QueryContext(ctx, query, a.HasFrom, a.From, a.HasTo, a.To)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

	items := make([]ProviderAnalyticsPeriodRecord, 0, 128)
	for rows.Next() {
		var x ProviderAnalyticsPeriodRecord
		if err := rows.Scan(&x.Provider, &x.PeriodStart, &x.SuccessCount, &x.SuccessNominal, &x.DepositAmount); err != nil {
			return nil, err
		}
