...
export default function OperatorManualAgentEntryPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setApplications((Array.isArray(applicationBody.items) ? applicationBody.items : []).filter((item) => item.applicant_data?.entry_source === "operator_manual"));
    } catch (loadError) {
  ...
  function openMigration(agent: AgentOption) {
  ...
  async function submit() {
    ...
    try {
      ...
      setNotice(`Migrasi ${body.credit_id || "data agent"} berhasil disimpan dan masuk ke Pengajuan Kredit.`);
      await loadData();
    } catch (submitError) {
    ...
    <main className="min-h-screen bg-[#eef7f2] p-3 text-[#082f28] sm:p-6 lg:p-8">
      <section className="mx-auto w-full max-w-6xl overflow-hidden rounded-[20px] border border-emerald-100 bg-white shadow-[0_16px_40px_rgba(6,78,59,0.08)]">
        ...
        <div className="overflow-x-auto border-t border-emerald-100">
          <table className="w-full min-w-[760px] text-left text-xs">
            ...
            <tbody>
              {visibleAgents.map((agent) => { const migration = migrationByMember.get(agent.id); return <tr key={agent.id} className="border-t border-slate-100 hover:bg-emerald-50/40"><td className="px-5 py-4"><p className="font-black text-slate-950">{agent.name || "Agent"}</p><p className="mt-1 text-[10px] font-semibold text-slate-400">{agent.email} · ID #{agent.id}</p></td><td className="px-4 py-4 font-semibold text-slate-600">{agent.phone || "-"}</td><td className="px-4 py-4 font-semibold text-slate-600">{agent.marketing_name || "Belum terhubung"}</td><td className="px-4 py-4">{migration ? <div><span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-black ${statusClass(migration.status)}`}>{statusLabel(migration.status)}</span><p className="mt-1 text-[10px] font-semibold text-slate-400">KRD-{String(migration.id).padStart(8, "0")} · {formatDate(migration.created_at)}</p></div> : <span className="text-xs font-bold text-slate-400">Belum dimigrasikan</span>}</td><td className="px-5 py-4 text-right"><button type="button" disabled={Boolean(migration)} onClick={() => openMigration(agent)} className="inline-flex h-9 items-center justify-center gap-2 rounded-xl bg-emerald-700 px-4 text-xs font-black text-white disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"><Upload className="h-4 w-4" />{migration ? "Sudah Migrasi" : "Migrasi Data"}</button></td></tr>; })}
      ...
      {selectedAgent ? <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-2 backdrop-blur-sm sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setSelectedAgent(null); }}>
        <section role="dialog" aria-modal="true" aria-labelledby="migration-title" className="flex max-h-[calc(100dvh-16px)] w-full max-w-3xl flex-col overflow-hidden rounded-[20px] bg-white shadow-[0_30px_90px_rgba(2,44,34,0.35)] sm:max-h-[calc(100dvh-40px)]">
          ...
          <footer className="shrink-0 border-t border-emerald-100 bg-white p-4 sm:px-6"><button type="button" disabled={saving || Boolean(preparing)} onClick={() => void submit()} className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 px-5 text-sm font-black text-white disabled:opacity-50">{saving ? <Loader2 className="h-5 w-5 animate-spin" /> : <CheckCircle2 className="h-5 w-5" />}{saving ? "Menyimpan Migrasi..." : "Simpan Migrasi Data"}</button></footer>
        </section>
      </div> : null}
    </main>
  );
