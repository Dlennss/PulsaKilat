...
export function RetailWithdrawClient({ authToken }: Props) {
  ...
  return (
    <div className="space-y-4 text-slate-950">
      ...
      {showCreateModal ? (
        <div className="fixed inset-0 z-50 bg-slate-950/50 px-3 py-5 backdrop-blur-sm">
          <div className="mx-auto flex h-full w-full max-w-md items-end sm:items-center">
            <div className="max-h-full w-full overflow-y-auto rounded-lg bg-white p-4 shadow-2xl">
              ...
              <form className="space-y-4 pt-4" onSubmit={submit}>
                <fieldset>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2">
                    {([
                      ...
                      return (
                        <button key={source.id} type="button" onClick={() => { setSourceType(source.id); setAmount(""); setError(""); }} className={active ? "rounded-lg border-2 border-emerald-600 bg-emerald-50 p-3 text-left" : "rounded-lg border border-slate-200 bg-white p-3 text-left"}>
                          ...
                          <span className="mt-1 block text-[10px] font-bold text-slate-500">{fmtIDR(source.balance)}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                <label className="block">
                  ...
                  <div className="mt-2 flex gap-2">
                    ...
                    <button type="button" onClick={() => setAmount(String(sourceBalance))} disabled={sourceBalance <= 0} className="h-11 rounded-lg border border-emerald-200 bg-emerald-50 px-3 text-[10px] font-black text-emerald-700 disabled:opacity-50">Semua</button>
                  </div>
                </label>
